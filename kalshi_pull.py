...
SERIES = [
    ...
    ("KXNFP",       "Jobs Report / Nonfarm Payrolls"),
...
def pull_markets_for_series(series_ticker):
    ...
    for _ in range(10):  # safety cap on pages
        ...
        for m in data["markets"]:
            ...
            volume = _first_not_none(m.get("volume"), m.get("volume_fp"))
            open_interest = _first_not_none(m.get("open_interest"), m.get("open_interest_fp"))
            markets.append({
                "ticker": m.get("ticker"),
                "title": m.get("title", ""),
